...
class integrated_patent_file_generator:
    ...
    def generate_sheet_1(self, abspaths:dict):
        ...
        for mid_class in abspaths:
            ...
            for f in abspaths[mid_class]:
                data = load_data(f, '국가 년도별 자료', self.settings['year_from'], self.settings['year_until'])
    ...
    def generate_sheet_2(self, abspaths:dict):
        for mid_class in abspaths:
            dflist_institution = []
            for f in abspaths[mid_class]:
                data = load_data(f, '출원인 년도별 자료', self.settings['year_from'], self.settings['year_until'])
                data.rename(columns = {'CODENAME':'CODE_NAME'}, inplace = True)
                data_indexed = generate_index_column(data)
                dflist_institution.append(data_indexed)
            df_institution = concat_dataframes(dflist_institution)
            result = drop_row_by(df_institution, 'PAN', self.settings["TPN_PAN_limit"])
    ...
    def generate_sheet_5(self, abspaths:dict):
        for mid_class in abspaths:
            dflist_yearly = []
            for f in abspaths[mid_class]:
                data = load_data(f, '소분류 전체 자료', self.settings['year_from'], self.settings['year_until'])
                data.rename(columns = {'CODENAME':'CODE_NAME'}, inplace = True)
